...
# 定义保存模型的路径
SAVE_PATH = os.path.join(current_folder_path,"saved_models/net.pth")
LOSS = np.inf
...
def train(path_imgs, path_labels, config, epochs=5):
    # 获得模型，默认是UNet，命令行-a显示声明AttentionUNet
    net = unet.Unet(config.filters, attention=config.att)
    ...
    for epoch in range(epochs):
        ...
        with progressbar.ProgressBar(
            ...
            for i in range(len(dataset_train)):  # boucle inf si on ne fait pas comme ça
                ...
                output, _ = net(imgs)  # 移除.cuda()
                loss_train = criterion(output, labels)  # 移除.cuda()
                loss_train.backward()
                optimizer.step()
        ...
        with progressbar.ProgressBar(
            ...
            for j in range(len(dataset_val)):
                bar2.update(j)
                output, _ = net(imgs)  # 移除.cuda()
                loss_val = criterion(output, labels)  # 移除.cuda()
...
def create_pred_dataset(path_img):
    ...
    # 处理多通道图像（如果存在）
    if len(img.shape) == 3:  # 如果是RGB图像
        img = img[:, :, 0]   # 取第一个通道（假设为灰度）
    
    img = np.array(img) / 255
    img = data.contrast_and_reshape(img)
    img = np.array(img).reshape(-1, 1, 256, 256)
    return img
...
def pred_(model, path_list):
    ...
    for path in path_list:
        ...
        res, att_map = model(torch.Tensor(img))  # 移除.cuda()
        res = np.array(res.detach().numpy())  # 移除.cpu()
        img_list.append(img.reshape(256, 256) * 255)
        pred_list.append((res > 0.5).astype(np.uint8).reshape(256, 256) * 255)
    return img_list, pred_list, att_map

def pred(model):
    model.eval()
    base_path = BASE_PATH + "test/images/"
    pathlist = [
        base_path + "0.png",
        base_path + "1.png",
        base_path + "2.png",
    ...
    utils.plot_att_map(imgs[-1], att_map.detach().numpy())  # 移除.cpu()

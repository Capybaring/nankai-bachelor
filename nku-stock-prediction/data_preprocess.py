...
def load_sentiment_resources(sentiment_file, negation_file, degree_file):
    ...
    with open(degree_file, "r", encoding="utf-8") as f:
        for line in f:
            ...
            if len(parts) < 2:
                print(
                    f"Warning: skipping malformed line in degree file: {line.strip()}"
                )
                continue
            try:
                ...
                print(
                    f"Warning: skipping line due to value error: {line.strip()} - {e}"
                )
                continue
...
def classify_and_score_words(word_list, sentiment_dict, negation_words, degree_dict):
    ...
    return calculate_weighted_score(
        sentiment_words, negation_words_positions, degree_words_positions, word_list
    )
...
def preprocess_and_merge(
    ...
    sentiment_dict, negation_words, degree_dict = load_sentiment_resources(
        sentiment_file, negation_file, degree_file
    )

    # 清洗评论文本并去除停用词
    df["processed_text"] = df["comment"].apply(
        lambda text: clean_and_segment(text, stopwords)
    )

    # 计算每条评论的情感得分
    df["sentiment_score"] = df["processed_text"].apply(
        lambda word_list: classify_and_score_words(
            word_list, sentiment_dict, negation_words, degree_dict
        )
    )
    ...
    if "date" in df.columns:  # 确保数据中有日期列
        ...
        market_df = pd.read_csv(
            market_data_file,
            usecols=["date", "code", "high", "low", "open", "close", "volume"],
        )
...
if __name__ == "__main__":
    stopwords_file = "./sentiment_dictionary/stopwords.txt"  # 停用词表文件
    user_dict_file = "./sentiment_dictionary/user_dict.txt"  # 自定义词典
    sentiment_file = "./sentiment_dictionary/sentiment_dict.txt"  # 情感词典
    negation_file = "./sentiment_dictionary/negation_words.txt"  # 否定词词典
    degree_file = "./sentiment_dictionary/degree_words.txt"  # 程度副词词典
    comment_file = "./stock_comment_data/hs300_c.csv"  # 评论数据文件
    stock_file = "./stock_comment_data/hs300_.csv"  # 市场数据文件

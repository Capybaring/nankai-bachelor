...
def make_generator_model(input_dim, output_dim, feature_size):
    model = Sequential([
        GRU(256, return_sequences=True, input_shape=(input_dim, feature_size)),
        GRU(128, return_sequences=False),
        Dense(64, activation='relu'),
        Dense(output_dim, activation='linear')  # 线性激活层用于回归任务
    ])
    return model
...
def make_discriminator_model(input_shape):
    model = Sequential([
        ...
        Dense(1, activation='sigmoid')  # 输出 0~1 概率
    ])
    return model
...
class GAN:
    def __init__(self, generator, discriminator, opt):
        ...
        self.generator_optimizer = tf.keras.optimizers.Adam(learning_rate=opt["lr"])
        self.discriminator_optimizer = tf.keras.optimizers.Adam(learning_rate=opt["lr"])
    ...
    def train_step(self, real_x, real_y):
        with tf.GradientTape() as gen_tape, tf.GradientTape() as disc_tape:
            ...
            real_y = tf.reshape(real_y, (-1, 1))  # 确保形状匹配
            fake_y = tf.reshape(fake_y, (-1, 1))
...
plt.plot(actual_prices, label="Real Close Price", color='blue')
plt.plot(predicted_prices, label="Predicted Close Price", linestyle="dashed", color='red')
plt.xlabel("Time")

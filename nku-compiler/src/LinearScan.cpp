...
{
    std::cout << "LinearScan constructor called." << std::endl;
    this->unit = unit;
    for (int i = 4; i < 11; i++)
        regs.push_back(i);
    for (int i = 21; i < 48; i++)
...
{
    std::cout << "allocateRegisters method called." << std::endl;
    for (auto &f : unit->getFuncs())
    {
        ...
        {
            std::cout << "Computing live intervals." << std::endl;
            computeLiveIntervals();
            std::cout << "Performing linear scan register allocation." << std::endl;
            success = linearScanRegisterAllocation();
            if (success)        // all vregs can be mapped to real regs
            {
                std::cout << "Modifying code." << std::endl;
                modifyCode();
            }
            else                // spill vregs that can't be mapped to real regs
            {
                std::cout << "Generating spill code." << std::endl;
                genSpillCode();
            }
        }
    }
...
{
    std::cout << "开始创建定义-使用链。" << std::endl;
    LiveVariableAnalysis lva;
    lva.pass(func); // 进行活跃变量分析
    std::cout << "活跃变量分析完成。" << std::endl;
    du_chains.clear();
    int i = 0;
    std::map<MachineOperand, std::set<MachineOperand *>> liveVar;
    for (auto &bb : func->getBlocks())
    {
        std::cout << "处理基本块：" << bb << std::endl;
        liveVar.clear();
        for (auto &t : bb->getLiveOut()) {
            liveVar[*t].insert(t); // 初始化基本块的liveOut集合
            std::cout << "liveOut变量：" << t<< std::endl;
        }
        int no;
        no = i = bb->getInsts().size() + i; // 设置指令编号
        std::cout << "基本块指令数量：" << bb->getInsts().size() << std::endl;
        for (auto inst = bb->getInsts().rbegin(); inst != bb->getInsts().rend(); inst++)
        {
            if(!*inst)
                continue;
            (*inst)->setNo(no--); // 逆序设置指令编号
            std::cout << "设置指令编号：" << (*inst)->getNo() << std::endl;
            for (auto &def : (*inst)->getDef())
            {
                if (def->isVReg())
                {
                    auto &uses = liveVar[*def];
                    du_chains[def].insert(uses.begin(), uses.end()); // 更新定义-使用链
                    std::cout << "更新定义-使用链，定义：" << def << std::endl;
                    auto &kill = lva.getAllUses()[*def];
                    std::set<MachineOperand *> res;
                    std::set_difference(uses.begin(), uses.end(), kill.begin(), kill.end(), std::inserter(res, res.end()));
                    liveVar[*def] = res; // 更新liveVar集合
                    std::cout << "更新liveVar集合，定义：" << def << std::endl;
                }
            }
            for (auto &use : (*inst)->getUse())
            {
                if (use->isVReg()) {
                    liveVar[*use].insert(use); // 更新liveVar集合
                    std::cout << "更新liveVar集合，使用：" << use<< std::endl;
                }
            }
        }
    }
    std::cout << "定义-使用链创建完成。" << std::endl;
}
void LinearScan::computeLiveIntervals()
{
    std::cout << "开始计算活跃区间。" << std::endl;
    makeDuChains();
    std::cout << "定义-使用链创建完成。" << std::endl;
    intervals.clear();
    for (auto &du_chain : du_chains)
    {
        ...
        for (auto &use : du_chain.second) {
            if (use->getParent() == nullptr) {
                std::cerr << "Error: use->getParent() is nullptr" << std::endl;
                continue;
            }
            t = std::max(t, use->getParent()->getNo());
        }
        if (du_chain.first->getParent() == nullptr) {
            std::cerr << "Error: du_chain.first->getParent() is nullptr" << std::endl;
            continue;
        }
    ...
    std::cout << "活跃区间的开始和结束位置调整完成。" << std::endl;
    ...
    std::cout << "重叠的活跃区间合并完成。" << std::endl;

    sort(intervals.begin(), intervals.end(), compareStart);
    std::cout << "活跃区间排序完成。" << std::endl;
    std::cout << "活跃区间计算完成。" << std::endl;
}

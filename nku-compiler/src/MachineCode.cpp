#include "../include/MachineCode.h" 
#include "Type.h"
#include <cstdint>
#include <cstring>
#include <iostream> // 添加用于 std::cout
...
{
    std::cout << "进入函数: MachineOperand::MachineOperand(int, int, bool, float)" << std::endl;
    this->type = tp;
...
{
    std::cout << "进入函数: MachineOperand::MachineOperand(std::string, bool, bool)" << std::endl;
...
{
    std::cout << "进入函数: MachineOperand::operator==" << std::endl;
    if (this->type != a.type)
        return false;
    if (this->type == IMM)
...
{
    std::cout << "进入函数: MachineOperand::PrintReg" << std::endl;
    switch (reg_no)
    {
    case 11:
        fprintf(yyout, "fp");
...
{
    std::cout << "进入函数: MachineOperand::output" << std::endl;
    /* HINT：print operand
    * Example:
    ...
    * lable addr_a -> print addr_a; */
    ...
    case IMM:
        ...
        if(is_float) {
            uint32_t temp;
            memcpy(&temp, &this->fval, sizeof(temp));
...
{
    std::cout << "进入函数: MachineInstruction::PrintCond" << std::endl;
    switch (cond)
    {
    case EQ:
        fprintf(yyout, "eq");
...
BinaryMInstruction::BinaryMInstruction(
    ...
    int cond)
{
    std::cout << "进入函数: BinaryMInstruction::BinaryMInstruction" << std::endl;
    this->parent = p;
...
{
    std::cout << "进入函数: BinaryMInstruction::output" << std::endl;
    switch (this->op)
    {
...
LoadMInstruction::LoadMInstruction(MachineBlock* p,
    ...
    int op, int cond)
{
    std::cout << "进入函数: LoadMInstruction::LoadMInstruction" << std::endl;
    this->parent = p;
...
{
    std::cout << "进入函数: LoadMInstruction::output" << std::endl;
    switch(this->op) {
    ...
    if(this->use_list[0]->isImm())
    {
        if(this->use_list[0]->isFloat()) {
            ...
            uint32_t val_int;
            memcpy(&val_int, &val, sizeof(val_int));
            fprintf(yyout, "=%u\n", val_int);
        }
        else {
...
StoreMInstruction::StoreMInstruction(MachineBlock* p,
    ...
    int op, int cond)
{
    std::cout << "进入函数: StoreMInstruction::StoreMInstruction" << std::endl;
    this->parent = p;
...
{
    std::cout << "进入函数: StoreMInstruction::output" << std::endl;
    switch(this->op)
    {
...
MovMInstruction::MovMInstruction(MachineBlock* p, int op, 
    ...
    int cond)
{
    std::cout << "进入函数: MovMInstruction::MovMInstruction" << std::endl;
    this->parent = p;
...
{
    std::cout << "进入函数: MovMInstruction::output" << std::endl;
    switch(this->op) {
...
BranchMInstruction::BranchMInstruction(MachineBlock* p, int op, 
    ...
    int cond)
{
    std::cout << "进入函数: BranchMInstruction::BranchMInstruction" << std::endl;
    this->parent = p;
...
{
    std::cout << "进入函数: BranchMInstruction::output" << std::endl;
    switch(op){
    case B:
        fprintf(yyout, "\tb");
        PrintCond();
...
CmpMInstruction::CmpMInstruction(MachineBlock* p, 
    ...
    int cond, int optype)
{
    std::cout << "进入函数: CmpMInstruction::CmpMInstruction" << std::endl;
    this->parent = p;
    this->type = optype;
    this->cond = cond;
    this->use_list.push_back(src1);
...
{
    std::cout << "进入函数: CmpMInstruction::output" << std::endl;
    switch(this->type) {
        case CMP:
            fprintf(yyout, "\tcmp ");
            break;
...
StackMInstruction::StackMInstruction(MachineBlock* p, int op, 
    ...
    int cond)
{
    std::cout << "进入函数: StackMInstruction::StackMInstruction" << std::endl;
    this->parent = p;
...
{
    std::cout << "进入函数: StackMInstruction::output" << std::endl;
    switch(op){
    case PUSH:
        fprintf(yyout, "\tpush {");
        break;
...
ZextMInstruction::ZextMInstruction(MachineBlock *p, MachineOperand *dst, MachineOperand *src, int cond) {
    std::cout << "进入函数: ZextMInstruction::ZextMInstruction" << std::endl;
    this->parent = p;
...
void ZextMInstruction::output() {
    std::cout << "进入函数: ZextMInstruction::output" << std::endl;
    fprintf(yyout, "\tuxtb ");
    def_list[0]->output();
    fprintf(yyout, ", ");
    use_list[0]->output();
...
VcvtMInstruction::VcvtMInstruction(MachineBlock* p,
                                   ...
                                   MachineOperand* src,
                                   int cond) {
    std::cout << "进入函数: VcvtMInstruction::VcvtMInstruction" << std::endl;
    this->parent = p;
...
void VcvtMInstruction::output() {
    std::cout << "进入函数: VcvtMInstruction::output" << std::endl;
    switch (this->op) {
...
VmrsMInstruction::VmrsMInstruction(MachineBlock* p) {
    std::cout << "进入函数: VmrsMInstruction::VmrsMInstruction" << std::endl;
    this->parent = p;
    this->type = MachineInstruction::VMRS;
}

void VmrsMInstruction::output() {
    std::cout << "进入函数: VmrsMInstruction::output" << std::endl;
    fprintf(yyout, "\tvmrs APSR_nzcv, FPSCR\n");
}

MachineFunction::MachineFunction(MachineUnit* p, SymbolEntry* sym_ptr) 
{ 
    std::cout << "进入函数: MachineFunction::MachineFunction" << std::endl;
    this->parent = p; 
...
{
    std::cout << "进入函数: MachineFunction::getSavedRRegs" << std::endl;
...
{
    std::cout << "进入函数: MachineFunction::getSavedFRegs" << std::endl;
    std::vector<MachineOperand*> ret;
    for(auto no : saved_regs){
        MachineOperand* reg = nullptr;
        if(no >= 16) {
...
{
    std::cout << "进入函数: MachineBlock::output" << std::endl;
    // 如果是一个空块则直接返回
...
{
    std::cout << "进入函数: MachineBlock::insertBefore" << std::endl;
...
{
    std::cout << "进入函数: MachineBlock::insertAfter" << std::endl;
    std::vector<MachineInstruction*>::iterator pos = find(inst_list.begin(), inst_list.end(), at);
    // 如果是最后一条
...
{
    std::cout << "进入函数: MachineFunction::output" << std::endl;
...
{
    std::cout << "进入函数: MachineUnit::PrintGlobalDecl" << std::endl;
    fprintf(yyout, "\t.data\n");
            ...
            if(var->arrayValues.empty()) {
                ...
                if(var->getType()->isIntArray() || var->getType()->isConstIntArray()) {
                    ...
                    for (auto value: var->arrayValues) {
                        ...
                        uint32_t temp;
                        memcpy(&temp, &tmp_value, sizeof(temp));
...
{
    std::cout << "进入函数: MachineUnit::output" << std::endl;
    // TODO
    /* Hint:
...
void MachineUnit::insertGlobalVar(IdentifierSymbolEntry *sym_ptr) {
    std::cout << "进入函数: MachineUnit::insertGlobalVar" << std::endl;
    global_var_list.push_back(sym_ptr);
}

void MachineUnit::PrintGlobal() {
    std::cout << "进入函数: MachineUnit::PrintGlobal" << std::endl;
